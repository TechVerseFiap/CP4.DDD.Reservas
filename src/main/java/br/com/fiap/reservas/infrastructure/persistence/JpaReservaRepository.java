...
import br.com.fiap.reservas.repository.ReservaRepository;
...
public interface JpaReservaRepository extends JpaRepository<Reserva, Long>, ReservaRepository {
    ...
    @Query("""
        ...
        where e.id = :equipamentoId
          and r.retirada < :entrega
          and r.entrega > :retirada
        """)
    boolean existeConflitoEquipamento(
            @Param("equipamentoId") Long equipamentoId,
            @Param("retirada") LocalDateTime retirada,
            @Param("entrega") LocalDateTime entrega);
